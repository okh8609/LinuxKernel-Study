...
static ssize_t drv_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
    ...
    if (mutex_lock_interruptible(&_device->lock))
        return -ERESTARTSYS;
...
static int drv_fasync(int fd, struct file *file, int on)
{
    ...
    ret = fasync_helper(fd, file, on, &_device->fasync);
...
static int __init simple_char_init(void)
{
    ...
    my_class = class_create(THIS_MODULE, "my_classs"); // 可在 /sys/class/my_classs/ 找到接下來創建的設備
    ...
    for (i = 0; i < MAX_DEVICES; i++)
    {
        ...
        _device->dev = device_create(my_class, NULL, MKDEV(dev, i), NULL, "my_dd:%d:%d", MAJOR(dev), i);
        printk("%s: create device: %d:%d\n", __func__, MAJOR(dev), MINOR(i));
        ...
        mutex_init(&_device->lock); // lock for procfs read access
    }
...
static void __exit simple_char_exit(void)
{
    ...
    for (i = 0; i < MAX_DEVICES; i++)
    {
        if (my_devices[i])
        {
            device_destroy(my_class, MKDEV(dev, i));
            if (my_devices[i]->kfifo_buffer)
                kfifo_free(my_devices[i]->kfifo_buffer);
            kfree(my_devices[i]->kfifo_buffer);
            kfree(my_devices[i]);
